...
from bert_dataset_model import BertClassifier, MyDataset
...
config = {
    ...
    'epochs': 5,
    'dropout': 0.3,
    'save_path': 'src\\models\\best_bert_classifier.pt'
}
train_data = pd.read_csv(config['train_path'])[['title', 'like_count_24h']]
val_data = pd.read_csv(config['val_path'])[['title', 'like_count_24h']]
train_data['labels'] = np.where(train_data['like_count_24h']>train_data['like_count_24h'].median(), 1, 0).reshape(-1, 1)
val_data['labels'] = np.where(val_data['like_count_24h']>train_data['like_count_24h'].median(), 1, 0).reshape(-1, 1)
